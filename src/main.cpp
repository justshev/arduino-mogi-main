...
/* =================== FAN SPEED CONFIG =================== */
#define FAN_SPEED_NORMAL  150   // PWM 0-255, kecepatan normal (suhu < 28°C)
#define FAN_SPEED_MAX     255   // PWM max (suhu >= 28°C)
#define FAN_TEMP_THRESHOLD 28.0f // batas suhu untuk max speed

/* =================== PUMP RELAY TIMING =================== */
#define PUMP_CYCLE_INTERVAL_MS  (8UL * 3600UL * 1000UL)  // 8 jam dalam ms
#define PUMP_ON_DURATION_MS     (10UL * 1000UL)           // pompa nyala 10 detik tiap siklus
...
/* =================== PUMP TIMER =================== */
unsigned long pumpCycleStartMs = 0;  // waktu mulai siklus pompa
bool pumpRunning = false;            // apakah pompa sedang nyala
...
void setup() {
  ...
  // Inisialisasi pump timer - pompa OFF dulu, nyala setelah 8 jam
  pumpCycleStartMs = millis();
  pumpRunning = false;
  digitalWrite(RELAY_PIN, HIGH);  // active LOW: HIGH = OFF
  relayState = false;
  Serial.println("[PUMP] Pompa OFF (menunggu siklus 8 jam)");
...
void loop() {
  ...
  // ========== PUMP RELAY (active LOW): nyala 10 detik tiap 8 jam ==========
  const unsigned long nowPump = millis();
  if (pumpRunning) {
    // Pompa sedang nyala, cek apakah sudah lewat durasi ON
    if (nowPump - pumpCycleStartMs >= PUMP_ON_DURATION_MS) {
      pumpRunning = false;
      relayState = false;
      digitalWrite(RELAY_PIN, HIGH);  // active LOW: HIGH = OFF
      Serial.println("[PUMP] Pompa OFF (selesai 10 detik)");
    }
  } else {
    // Pompa mati, cek apakah sudah 8 jam sejak siklus terakhir mulai
    if (nowPump - pumpCycleStartMs >= PUMP_CYCLE_INTERVAL_MS) {
      pumpCycleStartMs = nowPump;
      pumpRunning = true;
      relayState = true;
      digitalWrite(RELAY_PIN, LOW);   // active LOW: LOW = ON
      Serial.println("[PUMP] Pompa ON (siklus 8 jam)");
    }
  }
  ...
  if (now - lastSendMs >= sendIntervalMs) {
    ...
    // ========== FAN CONTROL berdasarkan suhu DS18B20 ==========
    if (tempOk) {
      if (tempDs >= FAN_TEMP_THRESHOLD) {
        pwmValue = FAN_SPEED_MAX;
        Serial.printf("[FAN] Suhu %.1f >= %.1f -> MAX speed (%d)\n", (double)tempDs, (double)FAN_TEMP_THRESHOLD, pwmValue);
      } else {
        pwmValue = FAN_SPEED_NORMAL;
        Serial.printf("[FAN] Suhu %.1f < %.1f -> Normal speed (%d)\n", (double)tempDs, (double)FAN_TEMP_THRESHOLD, pwmValue);
      }
    } else {
      // Kalau sensor DS gagal, pakai DHT sebagai fallback
      if (!isnan(tempDht) && tempDht >= FAN_TEMP_THRESHOLD) {
        pwmValue = FAN_SPEED_MAX;
      } else {
        pwmValue = FAN_SPEED_NORMAL;
      }
      Serial.printf("[FAN] DS gagal, fallback speed: %d\n", pwmValue);
    }
